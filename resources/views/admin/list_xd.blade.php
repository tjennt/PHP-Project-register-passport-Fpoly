...
<div class="container mt-4">
        ...
        <section>
                ...
                <div class="table-responsive-sm mt-3 " id="list_xd2">
                        <table class="table table-hover table-light table-nowarp ">
                            ...
                            <tfoot>
                                    @foreach($result_xd as $item)
                                    <tr>
                                        <td>{{$item['id_hochieu']}}</td>
                                        <td>{{$item['hoten']}}</td>
                                        <td>{{$item['cmnd']}}</td>
                                        <td>{{$item['ngaycap']}}</td>
                                        <td><span class="text-warning">Chờ xét duyệt</span></td>

                                        <td @if(Auth::guard('admin')->id() == 5) style="display:none" @endif>
                                        <a class="btn btn-primary" href="admin/xetduyet/{{$item['id_congdan']}}" role="button">Xét duyệt</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tfoot>
            ...
            <div class="table-responsive-sm mt-3 " id="list_xd" style="display:none">
                <table class="table table-hover table-light table-nowarp ">
    ...
    <script type="text/javascript">
        $('#search').on('keyup',function(){
            $value=$(this).val();
            if($value != ""){
                document.getElementById("list_xd").style.display = "block";
                document.getElementById("list_xd2").style.display = "none";
                $.ajax({
                type : 'get',
                url : '{{URL::to('admin/show-xd/search')}}',
                data:{'search':$value},
                ...
                document.getElementById("list_xd").style.display = "none";
                document.getElementById("list_xd2").style.display = "block";
            }
        })
        </script>
        <script type="text/javascript">
